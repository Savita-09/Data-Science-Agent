...
This release is a **single shared workspace**: private deployments require an application API key; every holder can access that workspace's datasets and analyses. Private production requires a secret of at least 32 characters. The Render demo explicitly enables `public_demo` and `ephemeral_storage`, allowing key-free access to separate `public-demo` data/model subdirectories. The API and worker resolve the same isolated paths; enabling demo mode does not expose the private workspace. All visitors share the demo's uploads, analyses, chat histories, and reports, as disclosed in the UI. Browser application credentials are stored in session storage only for private deployments; LLM credentials remain server-side. Add an identity-aware gateway, TLS, request-rate/concurrency controls, audit policy, storage quotas, and retention appropriate to private deployments. Tenant authorization, encryption at rest, and automatic data retention are not implemented here.
